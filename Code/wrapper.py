...
from Additional.DataReader import DataReader
# ---------------------------------------------------------------------------
# Quaternion helpers
# ---------------------------------------------------------------------------
...
def load_calibrate_and_sync(mat_number):
    ...
    imu_path = base / "Data" / "Data" / "Train" / "IMU" / f"imuRaw{mat_number}.mat"
    vicon_path = base /"Data"/ "Data" / "Train" / "Vicon" / f"viconRot{mat_number}.mat"
    params_path = base /"Data"/ "IMUParams.mat"
...
# ---------------------------------------------------------------------------
def integrate_gyro(ts, wx, wy, wz, R0_3x3 = None):
    ...
    if(R0_3x3 is None):
        euler = [0, 0, 0]
        current = R.from_euler('ZYX', euler)
    else:
...
def complementary_filter_angles(t_imu, wx, wy, wz, ax, ay, az, R0_3x3 = None):
    ...
    if(R0_3x3 is None):
        current_R = R.from_euler('ZYX', [0, 0, 0])
        comp_rpy[0] = [0, 0, 0]
    else:
...
def madgwick_filter(ts, wx, wy, wz, ax, ay, az, R0_3x3 = None):
...
# UKF estimator
# ---------------------------------------------------------------------------
def ukf_attitude_estimate(t_imu, ax, ay, az, wx, wy, wz, R0_3x3 = None):
    if(R0_3x3 is None):
        q0_wxyz = np.array([1, 0, 0, 0], dtype = float)
    else:
...
# Plotting
# ---------------------------------------------------------------------------
def plot_attitude_summary(mat_number, t_imu, ax, ay, az, wx, wy, wz, ukf_q, vicon_rotation = None):
    out_dir = Path(__file__).resolve().parent / "outputs"
    out_dir.mkdir(parents=True, exist_ok=True)

    if(vicon_rotation is None):
        
        gyro_rotation = integrate_gyro(t_imu, wx, wy, wz)
        ...
        comp_rpy = complementary_filter_angles(t_imu, wx, wy, wz, ax, ay, az)
        comp_yaw, comp_pitch, comp_roll = comp_rpy.T

        madgwick_rpy = madgwick_filter(t_imu, wx, wy, wz, ax, ay, az)
        ...
        plot_specs = [
            ("Roll",  gyro_roll, accel_roll, comp_roll, mad_roll, ukf_roll),
            ("Pitch",  gyro_pitch, accel_pitch, comp_pitch, mad_pitch, ukf_pitch),
            ("Yaw", gyro_yaw, accel_yaw, comp_yaw, mad_yaw, ukf_yaw),
        ]

        saved_mat = {'VICON': {}, 'GYRO': {}, 'ACCL': {}, 'COMP': {}, 'MADGWICK': {}, 'UKF': {}}

        for ax_i, (name, gyro_angle, accel_angle, comp_angle, mad_angle, ukf_angle) in zip(axes, plot_specs):
            ax_i.plot(t_imu, gyro_angle, label="Gyro-only", linewidth=1.5)
            saved_mat['GYRO'][name] = gyro_angle
            ax_i.plot(t_imu, accel_angle, label="Accel-only", linewidth=1.5)
            saved_mat['ACCL'][name] = accel_angle
            ax_i.plot(t_imu, comp_angle, label="Complementary", linewidth=1.5)
            saved_mat['COMP'][name] = comp_angle
            ax_i.plot(t_imu, mad_angle, label="Madgwick", linewidth=1.5)
            saved_mat['MADGWICK'][name] = mad_angle

            ax_i.plot(t_imu, ukf_angle, label="UKF", linewidth=1.5)
            saved_mat['UKF'][name] = ukf_angle
            ax_i.set_ylabel(f"{name} (rad)")
            ax_i.grid(True, alpha=0.3)
            ax_i.legend(loc="upper right")

        axes[-1].set_xlabel("Time (s)")
        fig.suptitle(f"Attitude Comparison (mat {mat_number}): Gyro, Accel, Complementary, Madgwick, and UKF")
        ...
        for ax_i, (name, vicon_angle, gyro_angle, accel_angle, comp_angle, mad_angle, ukf_angle) in zip(axes, plot_specs):
            ...
            saved_mat['VICON'][name] = vicon_angle
            ax_i.plot(t_imu, gyro_angle, label="Gyro-only", linewidth=1.5)
            saved_mat['GYRO'][name] = gyro_angle
            ax_i.plot(t_imu, accel_angle, label="Accel-only", linewidth=1.5)
            saved_mat['ACCL'][name] = accel_angle
    ...
    plt.close(fig)
    mat_path = out_dir / f"saved_mat_{mat_number}.mat"
    io.savemat(mat_path, saved_mat)
...
def main():
    ...
    parser.add_argument("--data_path", type = str, default = 'Data', help = "Location of data folder relative to where this python file is located.")
    parser.add_argument("--test_path", type = str, default = 'Test', help = 'Location of test folder relative to where this python file is located. ')
    args = parser.parse_args()

    if(args.mat_number < 7):
        #Running train, continue as per usual. 

        dr = DataReader(args.mat_number, args.data_path)
        t_imu = np.asanyarray(dr.imu_mat['ts'])
        ax, ay, az, wz, wy, wx = dr.get_all_imu_samples()
        vicon_rotation = dr.vicon_mat['rots']
        
        #t_imu1, ax1, ay1, az1, wx1, wy1, wz1, vicon_rotation = load_calibrate_and_sync(args.mat_number)
        ukf_q = ukf_attitude_estimate(t_imu, ax, ay, az, wx, wy, wz, vicon_rotation[0].as_matrix())
        plot_attitude_summary(args.mat_number, t_imu, ax, ay, az, wx, wy, wz, ukf_q, vicon_rotation)

        print(f"Processed mat {args.mat_number} with {len(t_imu)} IMU samples.")
    else:
        dr = DataReader(args.mat_number, args.data_path, True, args.test_path)
        t_imu = np.asanyarray(dr.imu_mat['ts'])
        ax, ay, az, wz, wy, wx = dr.get_all_imu_samples()

        ukf_q = ukf_attitude_estimate(t_imu, ax, ay, az, wx, wy, wz)
        plot_attitude_summary(args.mat_number, t_imu, ax, ay, az, wx, wy, wz, ukf_q)
